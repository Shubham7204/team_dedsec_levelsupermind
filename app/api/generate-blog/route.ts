...
export async function POST(req: NextRequest) {
  try {
    ...
    Now, based on the above instructions, generate a comprehensive blog post from the following content:
    ...
    if (!blogContent) {
      throw new Error('Generated blog content is empty')
    }
    ...
    if (error instanceof Error) {
      ...
      return NextResponse.json({ error: 'An unexpected error occurred while generating the blog' }, { status: 500 })
    }
  }
}